...
class MetricContext:
    def __init__(self, truth: np.ndarray, radius: int = 3):
        ...
        if labels.ndim != 2 or not isinstance(radius, int) or isinstance(radius, bool) or radius <= 0:
            raise ValueError("Need 2-D truth and a positive integer radius")
        if not np.isin(labels, [0, 1]).all():
            raise ValueError("Truth must be binary 0/1 or bool, with nodata removed explicitly")
        self.truth = labels.astype(bool)
        self.radius = radius
