...
def download_card(meta, compact=False):
    a = meta["artifact"]
...
<h2 id="download-heading">Your next submission, in one click.</h2><p>Build a single-band GeoTIFF from the new trained model. Every pixel is checked against the submission template.</p>
...
def build_home(meta, report, feed):
    board = feed.get("leaderboard", {})
    leader, tracked = board.get("leader", {}), board.get("tracked_user") or {}
    delta = report["artifact"]["new_pixel_field"]
    return f'''<div class="overview-heading"><div><div class="eyebrow">EXPERIMENT 001 <span class="eyebrow-line"></span> A NEW DIRECTION</div><h1>Find the faults.<br><span>Follow the evidence.</span></h1><p>A different approach to the GEMS Prize. Train, test and generate a submission—with a clear record of what we know, and what we don’t.</p></div><a class="text-link" href="executive_summary.html">Read the executive summary {icon('arrow')}</a></div>
<div class="hero-grid">{download_card(meta)}<section class="map-card" aria-label="Measured prediction overview"><div class="map-title"><span class="dot"></span> GEODAWN / NEVADA & CALIFORNIA <span class="mono">UTM 11N</span></div><div class="map-view"><div class="map-grid"></div><img src="assets/prediction-preview.png" alt="Actual Riftline confidence field across the GeoDAWN footprint; brighter traces mark model predictions, not verified faults"><span class="map-north">N<br>↑</span><span class="map-coordinates">100 M GRID<br>EPSG:32611</span></div><div class="map-legend"><span><i></i> Model-predicted traces</span><span>Max-pooled preview</span></div><div class="map-caption"><h3>A new field. Not a renamed file.</h3><p>{delta['different_valid_pixels']:,} valid pixels differ from the archived ensemble ({delta['different_fraction']:.1%}). These are predictions—not confirmed discoveries.</p><a href="experiments.html">Inspect the experiment {icon('arrow')}</a></div></section></div>
<div class="metrics-row"><article><span class="metric-label">PUBLIC LEADER</span><strong data-leader-score>{score(leader.get('score'))}</strong><small><span data-leader-name>{esc(leader.get('participant', 'Unknown'))}</span> · official snapshot</small></article><article><span class="metric-label">EXTRADR19 · BEST PUBLIC</span><strong data-user-score>{score(tracked.get('score'))}</strong><small>Account-level result, not artifact-linked</small></article><article><span class="metric-label">THIS CANDIDATE</span><strong class="not-scored">Unscored<span>↗</span></strong><small>Ready for an authenticated upload</small></article><article><span class="metric-label">LOCAL AUDIT · UNION DTI</span><strong>{score(report['audit']['union']['dti'])}</strong><small>Different labels. Not leaderboard-comparable.</small></article></div>
...
<article><span>2</span><div><h3>Click “Build & download .tif” above.</h3><p>The browser checks the pinned field and reference mask, writes a GeoTIFF, reads it back, then downloads it only if all checks pass. If unsupported, use “Direct validated TIF.” Do not upload this web page, a PDF, or an evidence JSON.</p></div></article>
...
<article><span>4</span><div><h3>Paste the suggested Note.</h3><p>Use the copy button above. The note identifies the strategy, policy, seed and artifact. A browser build adds its own unique build identity. Keep the filename and hash with the result.</p></div></article>
...
def build_experiments(meta, report):
    rows = []
    ...
    p = report["partition"]
    return heading("EXPERIMENT LOG / 001", "A hypothesis is not a result.", "Three executed arms. One pre-registered selection rule. Every local number below is a published-catalogue surrogate—not a hidden-label or leaderboard score.") + f'''
...
def build_research(report):
    return heading("RESEARCH LOG", "Better questions.<br>Falsifiable experiments.", "The aim is genuine fault discovery. Each proposed upgrade has a rationale, a test and a reason it could fail—not a promised score.") + f'''
...
<article class="white-card"><span class="eyebrow">04 / SOURCE INDEPENDENCE</span><h2>Don’t learn the answer key.</h2><p>The historical SGMC pseudo-label experiments reuse a published source for training and proxy scoring. Geography helps, but does not remove source circularity.</p><div class="tag">Implemented: proxy never used in training</div><p class="small">SGMC still informs tuning in this experiment. A distinct-source audit remains a separate requirement, not a completed check.</p><a href="../legacy/data/evidence/pseudo_labels/pooled_two_population_contrast.json" class="text-link">Read the inherited limitation ↗</a></article></section>
...
def build_verification(meta, report, review):
...
<section class="section-block"><div class="table-scroll"><table><caption>Read-back of the published direct artifact against the hash-pinned supplied template.</caption><thead><tr><th>Gate</th><th>Requirement</th><th>Measured detail</th></tr></thead><tbody>{gates}</tbody></table></div></section>
...
def main():
    ...
    pages = [("index.html", "Mission control", build_home(meta, report, feed)),
             ("executive_summary.html", "Executive summary", build_summary(meta, report)),
             ("experiments.html", "Experiments", build_experiments(meta, report)),
             ("sources.html", "Data & sources", build_sources(data, sources, feed)),
             ("research.html", "Research log", build_research(report)),
             ("verification.html", "Verification", build_verification(meta, report, review))]
