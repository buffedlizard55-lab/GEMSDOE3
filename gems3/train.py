...
from .common import MODEL_SOURCES, ROOT, code_digest, read_json, sha256, utc_now, write_json
...
def run(config_path: Path, data_dir: Path, output_dir: Path) -> dict:
    ...
    for name in MODEL_SOURCES:
        shutil.copyfile(ROOT / "gems3" / name, source_dir / name)
    ...
    if code_digest() != fingerprint:
        raise RuntimeError("Model code changed during this run; refusing a misleading source identity")
    ...
    report = {"schema_version": 1, "started_at": started, "completed_at": utc_now(),
              ...
              "config": config, "config_sha256": sha256(config_path), "code_sha256": fingerprint, "source_bundle": "source",
              ...
              "partition": {"block_px": config["block_px"], "buffer_px": config["buffer_px"],
                            ...
                            "fold_rule": f"((row//{config['block_px']})*3 + (col//{config['block_px']})*5) % 4",
