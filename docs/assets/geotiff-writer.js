/*
 * Generic TIFF mechanics reused from the pinned upstream legacy/docs/geotiff_writer.js.
 * Attribution and archive identity: THIRD_PARTY.md and provenance/import.json.
 *
 * Riftline calls buildGeoTIFF with its NEW, hash-verified model field. It does not use the
 * legacy binary-RLE prediction payload or generateFromPayload path below.
 *
 * Classic little-endian, single-band float32 TIFF; zlib-wrapped Deflate (tag 8) when
 * CompressionStream is available, otherwise uncompressed (tag 1). GeoTIFF EPSG:32611,
 * PixelIsArea, scale/tiepoint, and explicit NaN nodata are re-read and checked.
 * The same writer is exercised under Node and actual Chromium, with independent Rasterio
 * read-back. This description is not a claim that a hosted CI run has already occurred.
 *
 * Container bytes may differ from the Python artifact. Decoded model pixels must not.
 */
(function (global) {
  'use strict';
