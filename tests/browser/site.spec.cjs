...
test('home makes the real file obvious and all navigation works', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Your next submission, in one click.' })).toBeVisible();
...
test('source search and type filtering are functional', async ({ page }) => {
  ...
  await expect(page.locator('#source-table tbody tr:visible')).toHaveCount(15);
});
...
test('browser receipt and subsequent direct download retain the correct identities', async ({ page }) => {
  ...
  await expect(page.locator('.file-details .hash')).toContainText(receipt.sha256);
  ...
  await expect(page.locator('.file-details .hash')).toContainText(meta.artifact.sha256);
});
