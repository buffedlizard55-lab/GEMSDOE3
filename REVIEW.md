...
- Release state: **PR #8 merged into `main`** as merge commit `d2c82e1` on 2026-09-25, after pull-request CI [36166373698](https://github.com/buffedlizard55-lab/GEMSDOE3/actions/runs/36166373698) passed (ruff, 148 Python tests, artifact checks, staged pages and the real Chromium suite) on head `c7c5e72`. [Post-merge CI](https://github.com/buffedlizard55-lab/GEMSDOE3/actions/runs/36166554368) and the [Pages deployment](https://github.com/buffedlizard55-lab/GEMSDOE3/actions/runs/36166553440) both succeeded, and the deployed manifest and home page were read back through the research tool (`evidence/pages-deployment-v4.json`). A mid-session GitHub authentication failure delayed this release; it is recorded in `evidence/github-access.json` and was resolved by reconnecting GitHub, not by weakening any check. Byte-level verification of the deployed files is the job of the scheduled workflow's `verify_live` step; raw HTTPS to the Pages host is blocked from this sandbox and is not claimed. Deployment is not a score.
